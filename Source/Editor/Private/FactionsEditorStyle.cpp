...
#define IMAGE_PLUGIN_BRUSH(RelativePath, ...) \
	FSlateImageBrush(FFactionsEditorStyle::InContent(TEXT(RelativePath), ".png"), __VA_ARGS__)
#define BOX_PLUGIN_BRUSH(RelativePath, ...) \
	FSlateBoxBrush(FFactionsEditorStyle::InContent(TEXT(RelativePath), ".png"), __VA_ARGS__)
#define IMAGE_BRUSH(RelativePath, ...) \
	FSlateImageBrush(StyleSet->RootToContentDir(RelativePath, TEXT(".png")), __VA_ARGS__)
#define BOX_BRUSH(RelativePath, ...) \
	FSlateBoxBrush(StyleSet->RootToContentDir(RelativePath, TEXT(".png")), __VA_ARGS__)
#define TTF_PLUGIN_FONT(RelativePath, ...) \
	FSlateFontInfo(FFactionsEditorStyle::InContent(TEXT(RelativePath), ".png"), __VA_ARGS__)
#define TTF_CORE_FONT(RelativePath, ...) \
	FSlateFontInfo(StyleSet->RootToCoreContentDir(RelativePath, TEXT(".ttf")), __VA_ARGS__)
...
void FFactionsEditorStyle::Create()
{
	...
	StyleSet->Set("Relation.DirectionalCheckBox",
		FCheckBoxStyle()
	...
	StyleSet->Set("Relation.TransparentRoundedBox",
		new FSlateRoundedBoxBrush(FStyleColors::Transparent, CheckboxCornerRadius, Icon20x20));
}
